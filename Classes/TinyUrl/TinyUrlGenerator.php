   ...
   public  $optionUrlKey = false;
    ...
    protected function generateNewTinyurl($targetUrl, $targetUrlHash)
    {
        ...
        if ($customUrlKey === false) {
            ...
            $this->getDatabaseConnection()->exec_UPDATEquery('tx_tinyurls_urls', 'uid=' . $insertedUid,
                array('urlkey' => $tinyUrlKey));
    ...
    protected function getCustomUrlKey($targetUrlHash)
    {
        ...
        $customUrlKeyWhere = 'urlkey=' . $this->getDatabaseConnection()->fullQuoteStr($customUrlKey,
                'tx_tinyurls_urls');
     ...
     */
    public function getExistingTinyurl($targetUrlHash)
    {

        $whereStatement = 'target_url_hash=' . $this->getDatabaseConnection()->fullQuoteStr($targetUrlHash,
                'tx_tinyurls_urls');
    ...
    public function getTinyUrlDataById($id)
    {
        $whereStatement = 'uid = '.$id;
    ...
    public function getTinyUrlById($tinyUrlKey)
    {
        if ($this->configUtils->getExtensionConfigurationValue('createSpeakingURLs')) {
            $tinyUrl = $this->urlUtils->generateTinyurlHash($tinyUrlKey);
        } else {
